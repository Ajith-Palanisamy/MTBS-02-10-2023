...
$(document).ready(function(){
      ...
      success: function(data) {
        ...
        data.forEach(function(item) {
          var listItem = $('<li>');
           var linkItem=$('<a>')
            .text(item.name)
            .attr("class","movieLink transitionLink")
	...
	$(document).on('click','.movieLink',function(event)
	  {
		...
		localStorage.setItem('movie_id',selected_movie.movie_id);
		localStorage.setItem('movie_name',selected_movie.name);
		...
		$.ajax({
           ...
           // table.find("tr:not(:first)").remove();
            ...
            $.each(data, function(index, object) {
                ...
                .attr("href","javascript:void(0)")
                .attr("class","bookTicket transitionLink")
                .text("Book Tickets");
                var td=$("<td>").append(l);
                row.append(td);
                table.append(row);
	...
	$(document).on('click','.bookTicket',function(event)
	{
		...
		localStorage.setItem('show_id',$(this).attr("show_id"));
   ...
   $(document).on('click', '#confirm-button', function(event) 
   {
    	...
    	data.show_id=localStorage.getItem("show_id");
